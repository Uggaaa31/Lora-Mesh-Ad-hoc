/*
 * WebConfig.h - Konfigurasi LoRa + pilihan WiFi Gateway via Web Browser
 * Mode : WiFi AP+STA
 * Port : 80
 ...
 * - Halaman config hanya memilih profil WiFi mana yang dipakai.
 * - SSID config gateway bernama LoRa-Gateway-CFG tanpa password.
 * - Tidak ada scan SSID lain di sekitar gateway.
 */
#pragma once
#include <WiFi.h>
#include <WebServer.h>
...
<div class="fld"><label>Pilihan WiFi dari secrets.h</label>
<select name="wprof">%%WIFI_OPTIONS%%</select>
...
static bool _load(LoRaRuntimeCfg& out) {
    ...
    bool has = _prefs.isKey("sf") || _prefs.isKey("bw") || _prefs.isKey("wprof");
    if (has) {
        out.sf             = _prefs.getUChar("sf", out.sf);
        out.bwKHz          = _prefs.getUInt("bw", out.bwKHz);
        out.wifiProfileIdx = _sanitizeProfileIndex(_prefs.getUChar("wprof", out.wifiProfileIdx));
    }
    _prefs.end();
    return has;
}
...
static String _buildWiFiOptions() {
    String options;
    for (size_t i = 0; i < WIFI_PROFILE_COUNT; ++i) {
        bool selected = (i == getWiFiProfileIndex(_cfg));
        String label = String(i + 1) + ". " + WIFI_PROFILES[i].ssid;

        options += "<option value=\"" + String(i) + "\"";
        if (selected) options += " selected";
        options += ">" + _htmlEscape(label) + "</option>";
    }
    return options;
}
...
static String _build() {
    ...
    s.replace("%%WIFI_OPTIONS%%", _buildWiFiOptions());
...
static void _onConfigSave() {
    ...
    if (_srv->hasArg("wprof")) _cfg.wifiProfileIdx = _sanitizeProfileIndex(_srv->arg("wprof").toInt());
    ...
    _prefs.putUChar("wprof", _cfg.wifiProfileIdx);
    _prefs.remove("cr");
    _prefs.remove("txpwr");
    _prefs.remove("ufo");
    _prefs.remove("wssid");
